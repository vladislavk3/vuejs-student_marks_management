        ...
        <el-row>
            <el-col :span="24" style="position: relative;">
                ...
                <el-form-item label="科目:" class="table-view">
                    <el-checkbox size="small" label="总分" v-model="filters.total" @change="onChangeTotal" style="float:left;"></el-checkbox>
                    <el-checkbox-group v-model="filters.checkedsubjects" class="inline" @change="onChangeSubject">
                        <el-checkbox size="small" v-for="(item, idx) in intersectSubjects" :label="item" :key="idx">{{ item.dictname }}</el-checkbox>
                    </el-checkbox-group>
    ...
    export default {
        ...
        computed: {
            intersectSubjects() {
                ...
                this.filters.checkedsubjects = result;
        ...
        methods: {
            isSearchable() {
                return !(this.filters.subfilter[0].examidname=='' || 
                        this.filters.subfilter[1].examidname=='' || 
                        this.intersectSubjects.length==0 || 
                        (this.filters.checkedsubjects.length == 0 && this.filters.total == false));
            },
            getGrades() {
            ...
            onChangeAllTotal() {
                if (this.total) {
                    ...
                    this.filters.checkedsubjects = this.intersectSubjects;
                } else {
            ...
            onChangeSubject() {
                if (this.filters.checkedsubjects.length == this.intersectSubjects.length && this.filters.total) {
                    this.total = true;
                }
            ...
            onAction(message) {
                ...
                this.$emit(message, this.filters.gradeidname.split(':')[0], this.filters.gradeidname.split(':')[1], this.filters.classids.filter(item=>item!=''), this.classes, this.filters.subfilter[0].examidname.split(':')[0], this.filters.subfilter[0].examidname.split(':')[1], this.filters.subfilter[1].examidname.split(':')[0], this.filters.subfilter[1].examidname.split(':')[1], this.filters.total, this.filters.checkedsubjects);
            }
        },
        created() {
            this.getGrades();
